...
const Marginalia = ({ id, text, theme }) => {
  ...
  return (
    <span className="relative inline-block group">
      <button
        ...
        className={`align-super text-xs font-bold px-1 rounded cursor-pointer transition-colors ${isOpen ? theme.colors.highlight : 'text-gray-400'} hover:${theme.colors.highlight}`}
      >
        [{id}]
      </button>
      ...
      <div
        className={`
                    absolute left-1/2 bottom-full mb-2 -translate-x-1/2 w-64 
                    p-4 text-sm font-normal leading-relaxed z-50
                    transition-all duration-200 ease-out origin-bottom
                    ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}
                    ${theme.colors.card} ${theme.border} ${theme.shadow}
                `}
      >
        <div className={`font-bold mb-1 text-xs uppercase tracking-wider ${theme.colors.highlight} font-sans`}>Note {id}</div>
        <div
          className={`${theme.colors.text} font-bold italic tracking-wide`}
        ...
        {/* Connector Arrow */}
        <div className={`absolute top-full left-1/2 -translate-x-1/2 -mt-[2px] w-3 h-3 rotate-45 border-b-2 border-r-2 ${theme.border.split(' ')[0]} ${theme.colors.card}`}></div>
      </div>
    </span>
  );
};
...
const parseContent = (content, footnotes, theme) => {
  ...
  return parts.map((part, index) => {
    ...
    if (match) {
      const id = match[1];
      return <Marginalia key={index} id={id} text={footnotes[id]} theme={theme} />;
    }
    return part;
  });
};
  ...
  <div className="grid grid-cols-1 md:grid-cols-12 gap-8 mb-24 items-center animate-in fade-in duration-700">
    <div className="md:col-span-7 space-y-6">
      ...
      <div className="flex flex-wrap gap-4 pt-4">
        <BrutalButton theme={theme} color={theme.colors.primary} onClick={() => navigate('experiments')}>
...
const BlogPost = ({ post, onBack, theme }) => {
  const [readMode, setReadMode] = useState('deep');

  return (
    <div className="max-w-4xl mx-auto animate-in slide-in-from-bottom-8 duration-500">
      <button onClick={onBack} className="mb-8 font-bold flex items-center gap-2 hover:underline">
...
const MazeGame = ({ theme }) => {
  ...
  useEffect(() => {
    ...
    loop();
    return () => { window.removeEventListener('deviceorientation', ori); cancelAnimationFrame(af) };
  }, [started, permission, fullScreen, theme]);

  return (
    <div className="relative w-full h-80 group perspective-1000">
      {!started ? (
        <div className={`absolute inset-0 flex flex-col items-center justify-center ${theme.colors.bg} ${theme.border} z-10 text-center p-4`}>
          <Smartphone size={48} className={`mb-4 ${theme.colors.highlight}`} />
          <h3 className={`text-2xl font-black mb-2 ${theme.colors.text}`}>GYRO MAZE</h3>
          <p className={`mb-6 max-w-sm ${theme.id === 'blueprint' ? 'text-blue-300' : 'text-gray-600'}`}>Tilt your phone to guide the marble. On desktop, use your cursor.</p>
          <BrutalButton theme={theme} onClick={start} color={theme.colors.accent} className="text-black">Start Experiment</BrutalButton>
        </div>
      ) : (
        <div ref={containerRef} className={`w-full h-full transition-transform ${fullScreen ? '' : theme.colors.bg}`}>
          <canvas ref={canvasRef} onClick={() => { if (gameState.current.won) handleRestart() }} className={`w-full h-full ${theme.colors.bg} ${theme.border} block`} style={{ touchAction: 'none' }} />
          <div className="absolute top-2 right-2 flex gap-2">
            <button onClick={handleRestart} className={`${theme.colors.card} ${theme.border} p-2 ${theme.colors.text}`}><RotateCcw size={16} /></button>
            <button onClick={() => { if (document.fullscreenElement) document.exitFullscreen(); else containerRef.current.requestFullscreen(); }} className={`${theme.colors.card} ${theme.border} p-2 ${theme.colors.text}`}><Maximize size={16} /></button>
          </div>
  ...
  <div className="animate-in zoom-in-95 duration-500">
    ...
    <div className="flex flex-col gap-12">
      ...
      <div className="w-full">
        ...
        <BrutalCard theme={theme} className="p-4"><LifeSim theme={theme} /></BrutalCard>
      </div>
      ...
      <div className="w-full">
        <div className={`flex justify-between items-end mb-4 border-b-2 ${theme.id === 'blueprint' ? 'border-blue-200' : 'border-black'} pb-2`}>
          <div>
            ...
            <h3 className="text-3xl font-black">Fitts's Law Playground</h3>
            <p className={`text-sm ${theme.id === 'blueprint' ? 'text-blue-300' : 'text-gray-600'}`}>Test the relationship between distance, size, and speed.</p>
          </div>
        </div>
...
const Shop = ({ theme }) => (
  <div className="text-center py-20 animate-in fade-in duration-500">
    <div className="inline-block relative">
      <ShoppingBag size={80} className={`mb-4 ${theme.id === 'blueprint' ? 'text-blue-900' : 'text-gray-300'}`} />
      <div className={`absolute top-0 right-0 ${theme.colors.secondary} ${theme.border} text-white text-xs font-bold px-2 py-1 transform rotate-12`}>SOON</div>
    </div>
    <h2 className="text-4xl font-black mb-4">THE SHOP</h2>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 animate-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className={`text-xl mb-6 ${theme.id === 'blueprint' ? 'text-blue-200' : ''}`}>Interested in collaboration, usability audits, or just want to chat about design systems?</p>
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3 text-lg font-bold">
            <Mail className={`${theme.border} p-1 ${theme.colors.card} ${theme.shadow}`} size={32} />
            hello@emilioharrison.com
          </div>
        </div>
      </div>
      <BrutalCard theme={theme}>
        {submitted ? (
          <div className="text-center py-12">
            <div className="text-4xl mb-4">🎉</div>
            <h3 className="text-2xl font-black mb-2">Message Sent!</h3>
            <button onClick={() => setSubmitted(false)} className={`mt-6 underline font-bold ${theme.colors.highlight}`}>Send another</button>
          </div>
        ) : (
          <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="space-y-4">
            <div><label className="block font-bold mb-2 uppercase text-sm">Email Address</label><input type="email" required className={`w-full ${theme.border} p-3 focus:outline-none ${theme.id === 'blueprint' ? 'bg-blue-900 text-white' : 'bg-white'}`} placeholder="you@example.com" /></div>
            <div><label className="block font-bold mb-2 uppercase text-sm">Message</label><textarea required rows={4} className={`w-full ${theme.border} p-3 focus:outline-none ${theme.id === 'blueprint' ? 'bg-blue-900 text-white' : 'bg-white'}`} placeholder="How can I help?" /></div>
            <BrutalButton theme={theme} type="submit" color={theme.colors.accent} className="w-full justify-center text-black">Send Message <Send size={18} /></BrutalButton>
          </form>
        )}
      </BrutalCard>
    </div>
...
export default function App() {
  ...
  const navigate = (tab) => { setActiveTab(tab); setReadingPost(null); setMobileMenuOpen(false); window.scrollTo(0, 0); };
  const navItems = [{ id: 'home', label: 'Home' }, { id: 'about', label: 'About' }, { id: 'blog', label: 'Notes' }, { id: 'experiments', label: 'Lab' }, { id: 'shop', label: 'Shop' }, { id: 'contact', label: 'Contact' }];
    ...
    <div className={`min-h-screen ${theme.colors.bg} ${theme.colors.text} ${theme.font} transition-colors duration-500 overflow-hidden relative`}>
      ...
      <header className={`sticky top-0 z-50 ${theme.colors.card} border-b-4 ${theme.id === 'blueprint' ? 'border-blue-200' : 'border-black'}`}>
        <div className="max-w-6xl mx-auto px-4 md:px-8 h-20 flex justify-between items-center relative z-10">
          <div onClick={() => navigate('home')} className={`text-2xl font-black tracking-tighter cursor-pointer hover:${theme.colors.highlight} transition-colors`}>EMILIO<span className={theme.colors.highlight}>.</span>HARRISON</div>
          <nav className="hidden md:flex gap-6 items-center">
            {navItems.map(item => (
              <button key={item.id} onClick={() => navigate(item.id)} className={`font-bold uppercase tracking-wide text-sm py-2 border-b-2 ${activeTab === item.id ? (theme.id === 'blueprint' ? 'border-blue-200 text-blue-100' : 'border-black text-black') : 'border-transparent opacity-60 hover:opacity-100'} transition-all`}>{item.label}</button>
            ))}
        ...
        {mobileMenuOpen && (
          <div className={`md:hidden border-t-2 ${theme.id === 'blueprint' ? 'border-blue-200' : 'border-black'} ${theme.colors.card} absolute w-full shadow-xl`}>
            {navItems.map(item => (
              <button key={item.id} onClick={() => navigate(item.id)} className={`block w-full text-left px-8 py-4 font-bold uppercase border-b ${theme.id === 'blueprint' ? 'border-blue-800' : 'border-gray-100'}`}>{item.label}</button>
            ))}
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 py-12 min-h-[60vh] relative z-10">
        {activeTab === 'home' && <><Hero navigate={navigate} theme={theme} /><BlogList onRead={(post) => { setReadingPost(post); setActiveTab('blog'); }} theme={theme} /></>}
        {/* About would go here, simplified for file size */}
        {activeTab === 'about' && <div className="text-center py-20"><SectionTitle theme={theme}>About</SectionTitle><p>UX Researcher & Creative Technologist.</p></div>}
        {activeTab === 'blog' && (readingPost ? <BlogPost post={readingPost} onBack={() => setReadingPost(null)} theme={theme} /> : <BlogList onRead={setReadingPost} theme={theme} />)}
        {activeTab === 'experiments' && <Experiments theme={theme} />}
        {activeTab === 'shop' && <Shop theme={theme} />}
