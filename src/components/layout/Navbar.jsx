        ...
        <header className={`sticky top-0 z-50 ${theme.colors.card} border-b-4 border-black`}>
            <div className="max-w-6xl mx-auto px-4 md:px-8 h-20 flex justify-between items-center relative z-10">
                ...
                <nav className="hidden md:flex gap-6 items-center">
                    {navItems.map(item => (
                        <a key={item.path} href={item.path} className={`font-bold uppercase tracking-wide text-sm py-2 border-b-2 ${isActive(item.path) ? 'border-black text-black' : 'border-transparent opacity-60 hover:opacity-100'} transition-all no-underline ${theme.colors.text}`}>{item.label}</a>
                    ))}
                </nav>
            ...
            {mobileMenuOpen && (
                <div className={`md:hidden border-t-2 border-black ${theme.colors.card} absolute w-full shadow-xl`}>
                    {navItems.map(item => (
                        <a key={item.path} href={item.path} onClick={() => setMobileMenuOpen(false)} className={`block w-full text-left px-8 py-4 font-bold uppercase border-b border-gray-100 text-black no-underline`}>{item.label}</a>
                    ))}
                </div>
            )}
        </header>
