    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {posts.map(post => (
                    <a key={post.slug} href={`/fieldnotes/${post.slug}`} className="no-underline block h-full">
                        <BrutalCard
                            theme={theme}
                        ...
                        >
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <span className={`text-sm font-medium ${theme.id === 'blueprint' ? 'text-blue-300' : 'text-gray-500'}`}>{post.data.date}</span>
                            </div>
