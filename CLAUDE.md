...
- **OpenRouter** (`createOpenRouterClient` in `src/lib/api-helpers.ts`, OpenAI-compatible client, `OPENROUTER_API_KEY`): used **only** for the initial photo-scan flow (`pages/api/parse-recipe.ts`), which reads a recipe-card photo in a single OCR call (ingredients + steps + headnote together, split back into the `_p: 1`/`_p: 2` payloads the streaming contract expects) and then a text-only structuring call. Both use a single model, `qwen/qwen3.5-flash-02-23` (flash tier — the earlier `qwen3.5-9b` took minutes on dense multi-column photos) — do not split vision/text models here again without updating this doc.
