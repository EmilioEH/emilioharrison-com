...
- **OpenRouter** (`createOpenRouterClient` in `src/lib/api-helpers.ts`, OpenAI-compatible client, `OPENROUTER_API_KEY`): used **only** for the initial photo-scan flow, which reads a recipe-card photo in a single OCR call (ingredients + steps + headnote together, split back into the `_p: 1`/`_p: 2` payloads the streaming contract expects) and then a text-only structuring call. The pipeline lives in `src/lib/services/parse-photo-core.ts` — provider-agnostic and free of Cloudflare/Astro imports, the same arrangement as `grocery-core.ts`, so the VM worker can run it for bulk photo import (`BULK-PHOTO-IMPORT-PLAN.md`). `pages/api/parse-recipe.ts` is only the NDJSON/ReadableStream wrapper plus auth and rate limiting: **new pipeline behaviour belongs in the core, not the route** — two independent photo parsers is exactly what `IMPORT-PIPELINE-V2-PLAN.md` was written to end. The core also handles a manually grouped multi-page spread (`transcribePhotos` takes several photos, reads each page with a continuation-page prompt, concatenates in page order, then structures once). Both use a single model, `qwen/qwen3.5-flash-02-23` (flash tier — the earlier `qwen3.5-9b` took minutes on dense multi-column photos) — do not split vision/text models here again without updating this doc.
  - **Every OpenRouter call sets `reasoning: { enabled: false }`** — the direct counterpart of Gemini's `thinkingBudget: 0` below, and it must not be dropped. Measured 2026-08-01 on one dense page (26 ingredients): with reasoning on, OCR ran 47–**177s** and structuring 18–**67s**, exceeding `OCR_TIMEOUT_MS` (100s) and `STRUCTURE_TIMEOUT_MS` (60s) respectively and failing the import outright; with it off, 25–28s and 12–14s. Every response was valid JSON with `finish_reason: "stop"` in both arms, so these failures were always the clock, never truncation. This is a **latency** fix only — a wider sweep across library photos did not show a transcription-accuracy difference between the two settings, and both misread small vulgar fractions (see `BULK-PHOTO-IMPORT-PLAN.md`, "Transcription accuracy is limited by image resolution").
- **Gemini** (`initGeminiClient` in `src/lib/api-helpers.ts`, `@google/genai`, `GEMINI_API_KEY`): used for everything else — `executeAiParse()` in `src/lib/services/ai-parser.ts` (AI Refresh and background Enhancement) and grocery list generation. Model: `gemini-3.1-flash-lite` (in `ai-parser.ts` and `grocery-core.ts` — keep the two in sync). The previous `gemini-2.5-flash` was retired for newly-created API keys (Google 404s it as "no longer available to new users"), so the model was moved to the newest flash-lite tier under a <$2/1M-output cost cap; bump to a full-flash model (e.g. `gemini-flash-latest`, ~5× the output price) if enhancement quality regresses. The dependency is pinned to `1.34.0` in `apps/recipes/package.json` because `@google/genai` v1.52.0 breaks the Cloudflare Workers runtime — do not bump it without verifying Workers compatibility.
...
- **Server-side background AI jobs**: Background Enhancement (`lib/services/recipe-enhancement-job.ts`) and grocery-list generation (`api/generate-grocery-list.ts`) run and persist their own results server-side via `context.locals.runtime.ctx.waitUntil(...)`, not a client-triggered fetch — this way the job survives the client backgrounding/closing the tab mid-generation. Both write progress/status directly to Firestore (`recipe.enhancementStatus`/`enhancementError`, `grocery_lists/*`'s `status`/`progress`/`message`) so the client only ever needs to read that document (a Firestore subscription for grocery lists, a bounded poll in `RecipeDetail.tsx` for recipes, since individual recipe docs have no live subscription) rather than owning the AI call. **Hard platform constraint**: Cloudflare cancels `waitUntil` work ~30 seconds after the response is sent — silently, without running catch blocks — so any AI call running under `waitUntil` MUST use a timeout that fires (and lets the error-status write land) comfortably inside that window (~25s; see `WAITUNTIL_SAFE_TIMEOUT_MS` in `recipe-enhancement-job.ts` and `GEMINI_TIMEOUT_MS` in `generate-grocery-list.ts`). A longer timeout there is not "more headroom" — it guarantees the job dies mid-flight with its status doc stuck at `processing` forever. In-request AI calls (photo import in `parse-recipe.ts`, AI Refresh) have no such cap while the client holds the connection and may run much longer. Gemini calls also set `thinkingConfig: { thinkingBudget: 0 }` — flash models' default dynamic thinking adds tens of seconds of pre-output latency these budgets can't afford. All AI endpoints (`parse-recipe`, `refresh`, `enhance`, `generate-grocery-list`) are rate-limited per user via `lib/rate-limit.ts` and bound each provider call with `lib/services/ai-timeout.ts` (combines a fixed timeout with the incoming request's abort signal via `AbortSignal`/`abortSignal`, so a hung call can't run forever and client-side cancellation actually stops the upstream request).
- **Background-worker cutover (`BACKGROUND_WORKER_ENABLED`)**: the two slow `waitUntil` jobs above (background enhancement, grocery generation) can instead be handed off to a self-hosted VM worker (`services/recipe-worker/`, see `BACKGROUND-JOBS-VM-PLAN.md`) that runs the same compute cores (`enhancement-core.ts`/`grocery-core.ts`) with no ~30s ceiling. The Cloudflare env flag `BACKGROUND_WORKER_ENABLED=true` (read via `isBackgroundWorkerEnabled` in `lib/env.ts`) switches both endpoints from "run under `waitUntil`" to "write a `pending` Firestore doc and return 202"; the worker's Firestore listeners claim `pending` docs transactionally and process them. Flag **off/absent is the default**, so the legacy `waitUntil` path (and the `WAITUNTIL_SAFE_TIMEOUT_MS`/`GEMINI_TIMEOUT_MS` budgets, which still govern it) stays authoritative and unchanged until the flag is set; flipping it back is an instant rollback with no redeploy. Two path-specific details: grocery enqueues the request's recipes as `inputRecipes` on the pending doc (the async worker never sees the original request); and the client-side timings that used to assume the 25s Cloudflare budget — the grocery stuck-detector (`grocery-stuck-detection.ts`) and the enhancement poll (`RecipeDetail.tsx`) — are sized for the worker's 120s `WORKER_JOB_TIMEOUT_MS` so a slow-but-healthy worker job isn't falsely flagged.
- **Bulk photo import runs on the same worker** (`services/recipe-worker/`, see `BULK-PHOTO-IMPORT-PLAN.md`), but for a different reason than the cutover above: phone browsers suspend JS and cancel in-flight fetches when the user switches apps, and a 15-photo batch takes minutes, so no flag makes a browser loop viable. The worker listens on `import_jobs` (status `pending`), reads the photos out of Firebase Storage with the service account it already holds — no new network path, no inbound port — and runs `parse-photo-core.ts`. Three rules that are load-bearing: the parsed recipe is stored **on the job doc, never in `recipes`** (a recipe exists only once the user reviews and accepts it, which is what keeps unreviewed OCR out of the library); import jobs go through a **concurrency limiter** (`WORKER_IMPORT_CONCURRENCY`, default 3) rather than the grocery listener's unbounded `void run(...)`; and each job's **batch counters move inside the job's own transaction**, since three concurrent jobs would otherwise race on the batch and the badge would disagree with the jobs it counts. Single-photo import is unaffected and still runs in-request on Cloudflare — the split is by count: `AiImporter` sends one photo down the existing in-request path and two or more to the queue. App-side surface: `POST /api/imports` (enqueue, 6 batches/hour), `GET /api/imports` (outstanding jobs + badge counts), `POST /api/imports/[jobId]` (accept/discard/retry); rules and limits live in `lib/services/import-batches.ts`. **Submitted photo keys are checked against the caller's id** — `/api/uploads` mints them as `{userId}-{timestamp}-{uuid}.{ext}`, so that prefix check is what stops a caller queueing a job that reads someone else's photo. The client **polls** `GET /api/imports` (on mount, on tab focus, every 8s while work is in flight) rather than subscribing: `firestore.rules` denies client reads on everything except `grocery_lists`.
