...
- **Robust Notification Subscription (Jan 2026)**: Implemented a resilient **Self-Healing** mechanism for push notifications in `PushNotificationManager`. The system now robustly handles race conditions by attempting to **auto-register the Service Worker** if it's missing when the user enables notifications, ensuring reliability even if the initial page load registration failed.
