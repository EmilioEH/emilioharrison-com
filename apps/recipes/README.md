...
2. **View a recipe** — full ingredients/instructions scaled to however many you're cooking for, a verdict & reviews, personal notes, share as text or PDF, and a manual "Refresh AI Data" action.
...
4. **Add a recipe to a week** — one tap adds/removes a recipe from the currently active week. No day-of-week picker — a recipe is either in this week's plan or it isn't. The library says which week it is adding to, and the card's button shows a check once the recipe is in it.
5. **Generate a grocery list** — for everything in the active week, with a Raw (deterministic) and Smart (AI) list mode, manual add/edit, check-off, and share/copy as text. Both read the normalised numeric amounts and both scale to this week's servings, so they never disagree about how much to buy.

   The stored list records the recipes (and servings) it was generated for, in `sourceRecipeIds`. That signature is what decides whether a regeneration is needed, replacing a test that asked "is there a document?" while the Firestore subscription was still loading — which answered "no" on nearly every page open, so the list rebuilt itself constantly, and each rebuild discarded the items the cook had ticked off, deleted or typed in. Regeneration is now non-destructive on both the Cloudflare and VM-worker paths: status and progress are written with `updateDocument`, and the finished list merges the cook's manual items, `archivedAt` and `unneededThisWeek` back on (`lib/grocery-signature.ts`). `useFirestoreDocument` reports `resolved` separately from `loading` so `data: null` means "no document" only once a snapshot has actually arrived.

6. **Decide what to cook** — a conversation that asks one thing at a time, offers options it has counted against your actual library, and lets you say what's wrong in words once it has shown you something (`SuggesterConversation.tsx`). It can also be told what's already in the fridge, and will prefer recipes that use it (`PantryPicker.tsx`, `lib/services/pantry-match.ts`). The week you just finished is reviewed in four taps per recipe (`WeekReviewPrompt.tsx`), which is what feeds the suggestions over time.
...
Cooking mode (step-by-step/timers), favorites, recipe version history, day-level meal scheduling, in-app feedback collection, push notifications & reminders, onboarding tutorials, bulk _edit_/select (bulk photo _import_ now exists — see below), H-E-B pricing integration, AI cost estimation, recurring grocery items, "reverse-engineer a dish from a photo" import, and admin user-impersonation/family-admin tooling. If you're looking for one of these in the code and can't find it, it was removed on purpose — see git history around the `cooking-app-scope-plan` branch for the rationale.
...
5. **No streaming.** With `thinkingBudget: 0` a turn lands in a second or two, so the first turn is _prefetched_ while the cook is still on the plan and previous turns stay on screen during a request. Revisit only if a turn is measured past ~3s.
...
- **Family-scoped** (`families/{familyId}/recipeData/{recipeId}`):
  - **Reviews**: a verdict (`disliked | ok | loved`, required) + optional comment + optional photo. Multiple reviews per user over time; edit history preserved.

    The five-star scale is gone. The week review asked in four taps and the recipe page in five stars, both wrote `source: 'quick'`, and a stored `4` therefore meant "Good" or "four out of five" with nothing on the record saying which — so the average shown on the overview ("4.3 ★") was over two incompatible scales. Both surfaces now ask the same three verdicts in the same words, and tag `source` at the point of writing (`week-review` / `detail`). `rating` is still written alongside `outcome` for one release so a rollback has something to read; readers fall back to `verdictForRating` when `outcome` is absent, so pre-migration reviews render correctly whether or not `scripts/migrate-reviews.ts` has been run. The mapping was chosen from the live data before any of this was written — see `scripts/audit-reviews.ts` and `scripts/REVIEW-AUDIT-2026-08-02.md`.

    The overview shows the **household's verdict**, not an average: each person's most recent word counts once, and loved + disliked renders as _mixed_ rather than being averaged away (`lib/household-verdict.ts`). Library cards carry a mark only for loved, disliked and mixed — nothing for "okay" or unrated, since most of a 413-recipe library is neither, and a mark on every card is the same mistake as the chef-hat placeholder that was removed from them.
  ...
  - **Week servings** (`weekPlan.servings`): how many this recipe is being cooked for _this week_. Absent means the recipe's own `servings`. It lives on the plan entry rather than on the recipe because cooking for six this week must not change the recipe for everyone forever — and it keeps the grocery contract intact, since the client still sends only recipe ids and `generate-grocery-list.ts` reads the count off the plan itself. `lib/servings-scale.ts` holds the rules: it multiplies `quantity` and nothing else, so a row with no number ("to taste") passes through untouched, and amounts written into instruction prose are deliberately never rewritten.
