...
- **Feedback System**: Directly submit bug reports and enhancement ideas from any screen via a persistent **Beta Footer Bar** or the global burger menu. Supports a global **keyboard shortcut** (`Cmd/Ctrl + Shift + F`) for instant reporting. The footer **smartly hides** when scrolling down to maximize screen real estate and reappears when scrolling up.
...
- **Quick Feedback Entry Points**: Implemented a persistent "Feedback" FAB and a global keyboard shortcut (`Cmd/Ctrl + Shift + F`) to streamline user reporting during beta testing. Integrated into the global layout with **automatic padding compensation** to ensure the footer never obscures page content.
