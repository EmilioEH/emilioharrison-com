...
- **Cooking Mode UX Polish**: Fixed a mobile responsiveness issue where the options menu was appearing off-screen. Implemented a responsive modal positioning pattern (`left-4 right-4 mx-auto ... sm:left-1/2`) that ensures full visibility and safe margins on small viewports while maintaining centered placement on desktop. Refined the post-cooking review flow with collapsible ingredient and step note sections for a cleaner interface.
