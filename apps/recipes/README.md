...
- **Robust Notification Subscription (Jan 2026)**: Implemented a resilient **Self-Healing** mechanism for push notifications in `PushNotificationManager`. The system now robustly handles race conditions by attempting to auto-register the Service Worker if missing, and ensures **Mobile API Compliance** by strictly using `registration.showNotification()` instead of the incompatible `Notification` constructor.
