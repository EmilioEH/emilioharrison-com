...
- **Robust Notification Subscription (Jan 2026)**: Implemented a resilient race-condition fix for push notifications in `PushNotificationManager`. The "Enable Notifications" button now intelligently waits for the Service Worker to be fully active/controlled before attempting subscription, providing an "Initializing..." feedback state to prevent errors on slower mobile devices.
