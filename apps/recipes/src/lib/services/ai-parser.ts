...
import { createOpenRouterClient } from '../api-helpers'
...
/**
 * High-level function to parse/enhance a recipe.
 * Used by /api/parse-recipe, /refresh, and background enhancement.
 */
...
export async function executeAiParse(
  ...
  const client = createOpenRouterClient(locals)
  ...
  // Build messages — convert image part to OpenAI format
  const messages: Array<{
    role: string
    content: string | Array<OpenAI.Chat.ChatCompletionContentPart>
  }> = [
  ...
  if (contentPart && typeof contentPart === 'object' && 'inlineData' in contentPart) {
    ...
    ;(messages[0].content as Array<OpenAI.Chat.ChatCompletionContentPart>).push({
      type: 'image_url',
