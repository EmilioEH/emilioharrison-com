...
test.describe('Advanced Features: Ratings, Favorites, and Editing', () => {
  ...
  test('should allow favoriting a recipe and filtering by favorites', async ({ page }) => {
    ...
    // 2. Open it
    await page.getByRole('button').filter({ has: page.getByRole('heading', { name: 'Other', exact: true }) }).click()
    const card = page.getByRole('button').filter({ has: page.getByRole('heading', { name: title }) })
    await card.click()
    ...
    await page.getByRole('button').filter({ has: page.locator('svg.lucide-x') }).click() // Close filter
  ...
  test('should update modification date on edit', async ({ page }) => {
      ...
      // 2. Open
      await page.getByRole('button').filter({ has: page.getByRole('heading', { name: 'Chicken', exact: true }) }).click()
      const card = page.getByRole('button').filter({ has: page.getByRole('heading', { name: title }) })
      await card.click()
