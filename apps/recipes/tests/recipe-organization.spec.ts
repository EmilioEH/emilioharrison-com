...
test.describe('Recipe Organization', () => {
  ...
  test('should organize recipes into protein folders', async ({ page }) => {
    ...
    await page.getByRole('button').filter({ has: page.locator('svg.lucide-plus') }).click() // Add Manual
    await page.getByPlaceholder("Grandma's Pancakes").fill('Chicken Parmesan')
    ...
    // "Chicken" folder text might be "Chicken" or "Chicken 1 Recipes" depending on layout
    // My FolderCard has title and count separately.
    const chickenFolder = page.getByRole('button', { name: /Chicken/i }).first()
    await expect(chickenFolder).toBeVisible()
    await expect(chickenFolder).toContainText('Recipes')
  ...
  test('should support "This Week" folder', async ({ page }) => {
    ...
    await page.getByPlaceholder("Grandma's Pancakes").fill('Weekly Loaf')
    await page.getByRole('button', { name: 'Save Recipe' }).click()
    
    // 2. Go to Uncategorized (since no protein)
    await page.getByRole('button', { name: 'Uncategorized' }).click()
    await page.getByText('Weekly Loaf').click()
