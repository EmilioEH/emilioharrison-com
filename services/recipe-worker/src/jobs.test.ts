...
function importDeps(
  ...
  return {
    ...
    fetchPhotos:
      ...
      vi.fn(async (_keys: string[]) => [
        { mimeType: "image/jpeg", data: "Zm9v" },
      ]),
    parsePhotos:
      overrides.parsePhotos ??
      vi.fn(
        async () =>
          ({ title: "Empanadas", servings: 4 }) as Record<string, unknown>,
      ),
...
describe("runGroceryForDoc", () => {
  ...
  it("persists an error status and logs it (and does not throw) when compute fails", async () => {
    ...
    expect(store.failGrocery).toHaveBeenCalledWith(
      "x",
...
describe("runImportForDoc", () => {
  ...
  it("never writes to the recipe library — the result stays on the job until reviewed", async () => {
    ...
    const written = (deps.store.completeImport as ReturnType<typeof vi.fn>).mock
      .calls[0][1];
  ...
  it("retries the parse once — per-job failure is normal here, not exceptional", async () => {
    ...
    const parsePhotos = vi.fn(async () => {
      call += 1;
      if (call === 1)
        throw new Error("Failed to structure the recipe from this photo.");
  ...
  it("fails loudly on the job after the retry is also spent", async () => {
    ...
    expect(fakeLogAiError).toHaveBeenCalledWith(
      "photo-import",
      expect.any(Error),
      {
        userId: "u1",
  ...
  it("does not burn the retry once the job budget has already expired", async () => {
    const parsePhotos = vi.fn(
      async (_client, _photos, opts: { externalSignal?: AbortSignal }) => {
        await new Promise((resolve) => setTimeout(resolve, 30));
        throw new Error(
          opts.externalSignal?.aborted ? "aborted" : "slow failure",
        );
      },
    );
