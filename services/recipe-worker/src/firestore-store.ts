...
export function createFirestoreStore(db: Firestore): WorkerStore {
  ...
  const finishImport = async (
    ...
    await db.runTransaction(async (tx) => {
      ...
      const completed =
        (batch.completed ?? 0) + (outcome === "complete" ? 1 : 0);
  ...
  return {
    async claimGrocery(listId) {
      ...
      return db.runTransaction(async (tx) => {
        ...
        return {
          ...
          sourceRecipeIds: Array.isArray(data.inputRecipeIds)
            ? data.inputRecipeIds
            : [],
        };
      });
    ...
    async writeGroceryProgress(listId, progress, message) {
      await groceryLists
        .doc(listId)
        .update({ progress, message, updatedAt: nowIso() });
    },
    ...
    async completeGrocery(listId, ingredients, sourceRecipeIds) {
      ...
      await db.runTransaction(async (tx) => {
        const snap = await tx.get(ref);
        const previous = (snap.data()?.ingredients ??
          []) as ShoppableIngredient[];
        tx.update(ref, {
    ...
    async claimImport(jobId) {
      ...
      return db.runTransaction(async (tx) => {
        ...
        const photoKeys = Array.isArray(data.photoKeys)
          ? data.photoKeys.filter(Boolean)
          : [];
    ...
    async reapStuckGrocery(deadlineMs, now) {
      ...
      for (const doc of snap.docs) {
        const claimedAt = (doc.data() as { groceryClaimedAt?: string })
          .groceryClaimedAt;
    ...
    async reapStuckImports(deadlineMs, now) {
      ...
      let count = 0;
      for (const doc of snap.docs) {
        const claimedAt = (doc.data() as { importClaimedAt?: string })
          .importClaimedAt;
        if (!isStale(claimedAt, now, deadlineMs)) continue;
