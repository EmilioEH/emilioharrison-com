...
export async function runGroceryForDoc(
  ...
  try {
    const ingredients = await deps.computeGrocery(deps.gemini, job.recipes, {
      ...
      onProgress: (update) =>
        deps.store
          ...
          .catch((e) =>
            console.warn(
              `[worker] grocery progress write failed (${listId}):`,
              e,
            ),
          ),
    });
    ...
    console.log(
      `[worker] grocery complete: ${listId} (${ingredients.length} items)`,
    );
    return "done";
  } catch (error) {
    const message =
    ...
    await deps.store
      ...
      .catch((e) =>
        console.error(`[worker] failGrocery(${listId}) write failed:`, e),
      );
    return "failed";
  }
}
...
export async function runImportForDoc(
  ...
  try {
      ...
      try {
        ...
        await deps.store.completeImport(jobId, {
          ...
          ...(partialFailure === "instructions"
            ? { partialFailure: "instructions" as const }
            : {}),
        });
    ...
    const message =
      error instanceof Error ? error.message : "Failed to import this photo";
    ...
    await deps.store
      ...
      .catch((e) =>
        console.error(`[worker] failImport(${jobId}) write failed:`, e),
      );
    return "failed";
  }
}
