...
function optionalInt(name: string, fallback: number): number {
  ...
  if (!Number.isFinite(n) || n <= 0) {
    throw new Error(
      `[recipe-worker] ${name} must be a positive number, got: ${raw}`,
    );
  }
  return n;
}
...
export function loadConfig(): WorkerConfig {
  ...
  try {
    ...
    throw new Error(
      "[recipe-worker] FIREBASE_SERVICE_ACCOUNT is not valid JSON",
    );
  }
  ...
  return {
    ...
    storageBucket:
      process.env.WORKER_STORAGE_BUCKET || `${projectId}.firebasestorage.app`,
